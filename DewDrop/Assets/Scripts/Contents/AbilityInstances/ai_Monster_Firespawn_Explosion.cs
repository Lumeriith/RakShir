...
public class ai_Monster_Firespawn_Explosion : AbilityInstance
{
    ...
    IEnumerator CoroutineExplode()
    {
        ...
        {
            List<Entity> targets = info.owner.GetAllTargetsInRange(transform.position, 1.25f, targetValidator);
            for(int i = 0; i < targets.Count; i++)
            {
                targets[i].statusEffect.ApplyStatusEffect(StatusEffect.Stun(stunDuration), this);
